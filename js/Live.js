fetch(
  "http://line.extraott-iptv.com:88/player_api.php?username=0485762425&password=3472153721&action=get_live_categories"
)
  .then((res) => res.json())
  .then((data) => {
    data.forEach((category) => {
      ...
      fetch(
        `http://line.extraott-iptv.com:88/player_api.php?username=0485762425&password=3472153721&action=get_live_streams&category_id=${category.category_id}`
      )
        .then((res) => res.json())
        .then((data) => {
          data.forEach((live) => {
            const newli = `
              <li class="card" data-id="${live.stream_id}">
              <div class="img">
                  <img src="${live.stream_icon}" alt="${live.name}" loading="lazy" >
              </div>
              <div class="info">
                  <h2 class="title">${live.name}</h2>
              </div>
          </li>
             `;
